...
//              Sprig.displaySurveyWithId(it)
              result.success("This feature is not yet available on android")
            }
          }
          "logout" ->  Sprig.logout()
          "presentDebugSurvey" ->  Sprig.presentDebugSurvey(mActivity)
          "setVisitorAttribute" ->  Sprig.setVisitorAttribute(call.argument<String>("key")!!,
  ...
  private fun track(event: String,  result: Result) {
    Sprig.track(event) { surveyState ->
      when (surveyState) {
        SurveyState.READY -> {
          // We received a survey for the event, present it to the user
          Sprig.presentSurvey(mActivity)
  ...
  private fun configure(env: String?, result: Result) {
    if(env != null) {
    Sprig.configure(this.mContext, env)
